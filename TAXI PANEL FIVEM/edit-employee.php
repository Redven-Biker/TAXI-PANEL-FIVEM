...
if (strlen($_SESSION['id']==0)) {
  header('logoff');
    ...
    if(isset($_POST['Submit']))
    {
      ...
      $role=$_POST['role'];
      $id=intval($_GET['id']);
      $result = $con->query("UPDATE accounts set name='$name', username='$username', rank='$rank', role='$role' where id='$id'");
      $msg = '<div class="alert alert-success" role="alert">Employee Edited</div>';
    }
...
<?php $result = $con->query("SELECT * from accounts WHERE id='".$_GET['id']."'"); while ($data = $result->fetch_assoc())  { ?>
